...
CLASS zcl_custom_signs IMPLEMENTATION.
  ...
  METHOD build_birthday_sign.
    DATA(word) = COND #( WHEN age < 50 THEN `young` ELSE `mature` ).
    result = |Happy Birthday! What a { word } fellow you are.|.
  ENDMETHOD.
  ...
  METHOD cost_of.
    DATA(cost) = 20 + 2 * strlen( sign ).
    result = |Your sign costs { cost }.00 { currency }.|.
  ENDMETHOD.
