...
{
    auto isSilence = !matchFirst(text, r"\S");
    if (isSilence)
        return "Fine. Be that way!";

    auto isYelling = matchFirst(text, r"[A-Z]") && !matchFirst(text, r"[a-z]");
    auto isQuestion = matchFirst(text, r"\?\s*$");

    return isYelling && isQuestion
        ? "Calm down, I know what I'm doing!" : isYelling
        ? "Whoa, chill out!" : isQuestion
        ? "Sure." : "Whatever.";
}
