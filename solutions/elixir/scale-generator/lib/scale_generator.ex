defmodule ScaleGenerator do
  ...
  def step(scale, tonic, step) do
    Stream.cycle(scale)
    |> Enum.at(start(scale, tonic) + @semitones[step])
  end
  ...
  def chromatic_scale(tonic \\ "C"),
    do: @sharp_scale |> start(tonic) |> generate(@sharp_scale)

  # Shift scale so requested notes are returned starting with given index
  @spec generate(integer, integer, [String.t()]) :: [String.t()]
  defp generate(offset, notes \\ 13, scale) do
    Stream.cycle(scale) |> Stream.drop(offset) |> Enum.take(notes)
  end
  ...
  def flat_chromatic_scale(tonic \\ "C"),
    do: @flat_scale |> start(tonic) |> generate(@flat_scale)
  ...
  def find_chromatic_scale(tonic),
