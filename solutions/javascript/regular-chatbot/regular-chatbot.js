...
export function removeEmoji(message) {
  const emojiEncryption = new RegExp(/emoji\d+/, "g");
  return message.replace(emojiEncryption, "");
}
...
export function checkPhoneNumber(number) {
  ...
  const phoneNumberFormat = /\(\+\d{2}\) \d{3}(?:-\d{3}){2}/;
  return phoneNumberFormat.test(number)
